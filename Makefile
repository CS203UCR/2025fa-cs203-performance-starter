...
microbench_cuda.exe: $(BUILD)perfstats.o $(BUILD)microbench_cuda.o microbench.cpp
	/usr/local/cuda/bin/nvcc -I/usr/local/cuda/include -L/usr/local/cuda/lib64 -DCUDA -lcuda -lcudart $(MICROBENCH_OPTIMIZE) microbench.cpp $(BUILD)perfstats.o $(BUILD)microbench_cuda.o --disable-warnings -o microbench_cuda.exe
